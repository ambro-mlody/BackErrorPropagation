import numpy as np

from neuralNetwork import NeuralNetwork
import pygame as py
import sys
import matplotlib.pyplot as plt
from examples import Examples, Point


def preprocess(x, xmin, xmax):
    x = np.array(x)
    xmin = np.array(xmin)
    xmax = np.array(xmax)
    return 0.1 + (x - xmin) / (xmax - xmin) * 0.8


def postprocess(x, xmin, xmax):
    x = np.array(x)
    return xmin + (x - 0.1) * (xmax - xmin) / 0.8


py.init()
size = (400, 500)
window = py.display.set_mode(size)
py.display.set_caption("Robot")

arm_length = 150
center = Point(0, size[1] / 2)
ex = Examples(center, arm_length)
training = ex.generate(1000)
point = Point(0, 0)
tempoint = Point(0, 0)
finalpoint = Point(0, 0)
nn = NeuralNetwork([2, 10, 10, 2], 1000, 0.0001)
xmin = min(training[0])
xmax = max(training[0])
ymin = min(training[1])
ymax = max(training[1])
training_x = preprocess(training[0], xmax, xmin)
training_y = preprocess(training[1], ymax, ymin)
nn.train(training_x, training_y)
for e in nn.errors:
    print(e)

while True:
    window.fill((60, 60, 60))
    for ev in py.event.get():
        if ev.type == py.QUIT:
            py.quit()
            sys.exit(0)
        if py.mouse.get_pressed()[0]:
            pos = py.mouse.get_pos()
            point = Point(pos[0], pos[1])
            out = nn.forward(preprocess([point.x, point.y], xmin, xmax))
            out = postprocess(out, ymin, ymax)
            tempoint = ex.translate(center, out[0])
            finalpoint = ex.translate(tempoint, np.pi - out[1] + out[0])
    tempoint.draw(window)
    finalpoint.draw(window)
    py.draw.line(window, "blue", center.tup(), tempoint.tup())
    py.draw.line(window, "yellow", tempoint.tup(), finalpoint.tup())
    point.draw(window)
    py.draw.circle(window, "green", (0, size[1] / 2), 8)
    py.display.update()

